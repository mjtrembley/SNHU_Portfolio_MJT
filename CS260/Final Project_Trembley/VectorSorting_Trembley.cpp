...
int partition(vector<Bid>& bids, int begin, int end) {
    ...
    while (!done) {
        ...
        //keep incrementing low value as long as it is less than pivot
        while (bids.at(low).title.compare(bids.at(pivot).title) < 0) {
            ++low;
        }
        //keep decrementing high value as long as it is more than pivot
        ...
        if (low >= high) {
            ...
            //swap low and high bids using built-in vector method
...
void quickSort(vector<Bid>& bids, int begin, int end) {
    ...
    //recursively acll quicksort using midpoint value (beginning to midpoint
...
void selectionSort(vector<Bid>& bids) {
    ...
    int min;
    //pos is position within the bids that marks sorted/unsorted
    for (unsigned int pos = 0; pos < bids.size(); ++pos) {
        ...
        if (min != pos) {
            //swap using built-in vector method
...
int main(int argc, char* argv[]) {
    ...
    while (choice != 9) {
        ...
        case 3:
            ...
            //time elapsed with result displayed
            ticks = clock() - ticks;
            cout << "time: " << ticks << "clock ticks" << endl;
            cout << "time: " << ticks * 1.0 / CLOCKS_PER_SEC << " seconds" << endl;
            break;
